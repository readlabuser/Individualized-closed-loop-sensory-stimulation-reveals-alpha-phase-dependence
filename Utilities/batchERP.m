...
for k = 1:length(fnames)
    ...
    % Wavelet transform
    [wt, f] = wavecwt(data(k).rawEEG,data(k).fs,params.freqs, params.cycles, params.steps);
    data(k).wt = wt;
...
%         data(k).thetaEEG = cat(1,tmp(mgdT+1:end,:),zeros(mgdT,size(tmp,2)));
    ...
    [waves,~] = wavecwt(data(k).rawEEG, data(k).fs, f(idx), c(idx), 1);
    ...
    for n = 1:length(data(k).kOn1)
        ...
        try
            ...
            data(k).cwt(:,:,:,m) = data(k).wt(data(k).kOn1(n)+[kWin(1):kWin(2)],:,:);
        catch
            fprintf('Whoa\n');
...
%     ERP.echtPhase
    ...
    ERP.cwt = cat(4, ERP.cwt,data(k).cwt);
