...
for k = 1:n
    ...
    for j = 1:2
        ...
        [wt(:,j,:,:), f] = wavecwt(noerp, fs, [1 70], [4 12], 140);
    end
    wt = permute(wt, [3 1 2 4]);

    % Get Alpha center frequency
    [~,IAF] = min(abs(dataTable(subIdx,:).IAF(1) - f));

    % Get induced power
    ind(k,:,:,1) = squeeze(mean(abs(squeeze(wt(pIdx{1},:,:,IAF))).^2,1));
    ind(k,:,:,2) = squeeze(mean(abs(squeeze(wt(pIdx{2},:,:,IAF))).^2,1));
...
plot(t,pitc(:,1),'k-')
hold on
fill(time, [pitc(:,1); ones(1,length(pitc(:,1)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
...
plot(t,pitc(:,2),'k-')
hold on
fill(time, [pitc(:,2); ones(1,length(pitc(:,2)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,pevo(:,1),'k-')
hold on
fill(time, [pevo(:,1); ones(1,length(pevo(:,1)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,pevo(:,2),'k-')
hold on
fill(time, [pevo(:,2); ones(1,length(pevo(:,2)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,ptot(:,1),'k-')
hold on
fill(time, [ptot(:,1); ones(1,length(ptot(:,1)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,ptot(:,2),'k-')
hold on
fill(time, [ptot(:,2); ones(1,length(ptot(:,2)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,pind(:,1),'k-')
hold on
fill(time, [pind(:,1); ones(1,length(pind(:,1)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
...
plot(t,pind(:,2),'k-')
hold on
fill(time, [pind(:,2); ones(1,length(pind(:,2)))'],'k')
xlim([0 max(t)])
ylim([0 0.05])
set(gca,'ydir','reverse')
xlabel('Time (ms)')
